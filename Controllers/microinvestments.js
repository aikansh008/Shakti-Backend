...
const scrapeData = async (url) => {
  const browser = await puppeteer.launch({
    headless: true,
    args: ['--no-sandbox', '--disable-setuid-sandbox']  // Required on EC2
  });
  ...
  try {
    ...
    await page.waitForSelector('.jsx-7cbb814d75c86232.resultbox_info', {
      timeout: 10000
    });
    ...
    const data = await page.evaluate(() => {
      ...
      boxes.forEach((box) => {
        const nameElement = box.querySelector('.store-name') || box.querySelector('h2');
